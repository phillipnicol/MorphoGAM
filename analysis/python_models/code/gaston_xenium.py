...
#Load csv data
coords = pd.read_csv("../../data/xenium_brain_coords.csv")
coords = coords.iloc[:, 1:]  # Remove the first column
np.save('../data/xenium_brain_coords.npy', coords)

print(coords.shape)

gene_expr = pd.read_csv("../../data/xenium_brain_counts.csv")
...
np.save('../data/xenium_brain_counts.npy', gene_expr)

print(gene_expr.shape)

# Load gene labels
gene_labels = pd.read_csv("../../data/xenium_brain_gene_names.csv")
gene_labels = gene_labels['x'].values
np.save("../data/xenium_brain_gene_names.npy", gene_labels)
...
counts_mat = np.load('../data/xenium_brain_counts.npy', allow_pickle=True)
coords_mat = np.load('../data/xenium_brain_coords.npy', allow_pickle=True)
gene_labels = np.load('../data/xenium_brain_gene_names.npy', allow_pickle=True)
...
np.save('../data/glmpca_xenium.npy', A)
...
plt.savefig('../plots/xenium_glmpca_components.png', dpi=300, bbox_inches='tight')
...
out_dir='xenium_brain_output' # folder to save model runs
optimizer = "adam"
num_restarts=30
...
gaston_model, A, S= process_NN_output.process_files('xenium_brain_output') # MATCH PAPER FIGURES
...
plt.savefig('../plots/xenium_isodepth_plot.png', dpi=300, bbox_inches='tight')
plt.close()  # Close the figure to free memory


#Save the isodepth values as csv
np.savetxt('../data/xenium_gaston_isodepth.csv', gaston_isodepth, delimiter=',')
...
# Create cell type information with proper numpy array formatting
cell_type_df = pd.DataFrame(index=range(len(gaston_labels)))
cell_type_df['cell_type'] = 'unknown'  # Default cell type

# Create cell type list as numpy array (important!)
ct_list = np.array(['unknown'])  # Convert to numpy array


counts_mat = counts_mat.T  # Now counts_mat is G x N

# compute piecewise linear fits
pw_fit_dict = segmented_fit.pw_linear_fit(counts_mat, gaston_labels, gaston_isodepth,
                                         cell_type_df=cell_type_df,  # Add this
                                         ct_list=ct_list,  # Add this
                                         zero_fit_threshold=zero_fit_threshold,
