...
rng = default_rng(42)  # Set seed for reproducibility
...
shuffled_indices = rng.permutation(len(coords))
coords = coords.iloc[shuffled_indices].reset_index(drop=True)
np.save('../data/coords_mat_shuffle.npy', coords)

print(coords.shape)

gene_expr = pd.read_csv("../../data/counts_sub_mucosa.csv")
gene_expr = gene_expr.iloc[:, 1:]  # Remove the first column
np.save('../data/counts_mat_shuffle.npy', gene_expr)

# Load gene labels
gene_labels = pd.read_csv("../data/mucosa_gene_names.csv")
gene_labels = gene_labels['x'].values
np.save("../data/gene_labels_shuffle.npy", gene_labels)

# Load saved matrices
counts_mat = np.load('../data/counts_mat_shuffle.npy', allow_pickle=True)
coords_mat = np.load('../data/coords_mat_shuffle.npy', allow_pickle=True)
gene_labels = np.load('../data/gene_labels_shuffle.npy', allow_pickle=True)
...
np.save('../data/glmpca_shuffle.npy', A)
...
plt.savefig('../plots/glmpca_components_shuffle.png', dpi=300, bbox_inches='tight')
...
out_dir='colorectal_tumor_tutorial_shuffle' # folder to save model runs
optimizer = "adam"
num_restarts=30
...
gaston_model, A, S= process_NN_output.process_files('colorectal_tumor_tutorial_shuffle') # MATCH PAPER FIGURES
...
plt.savefig('../plots/isodepth_plot_shuffle.png', dpi=300, bbox_inches='tight')
plt.close()  # Close the figure to free memory


#Save the isodepth values as csv
np.savetxt('../data/gaston_isodepth_shuffle.csv', gaston_isodepth, delimiter=',')
