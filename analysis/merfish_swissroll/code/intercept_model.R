...
df <- data.frame(gene = rownames(res1),
                 ...
                 beta2 = res2[,1],
                 beta3 = res3[,1],
                 pval = pmax(res1[,3], res2[,3], res3[,3]))

df$pval <- p.adjust(df$pval, method = "BH")
...
up.genes <- df |> mutate(max.min = beta1 + beta2 + beta3) |>
...
df <- t(as.matrix(Y2)[c("Mmp3", "Plek", "Nos3"),]) |>
  as.data.frame() |>
  mutate(t = fit2$xyt$t) |>
  pivot_longer(-t)
